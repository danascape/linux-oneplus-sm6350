...
#if defined(OEM_TARGET_PRODUCT_KEBAB) || defined(OEM_TARGET_PRODUCT_LEMONADES)
				...
				const union power_supply_propval *val)
{
	...
	if (chg->pd_active && chg->pd_active == val->intval) {
		/* @bsp, 2020/08/27, add to detect SVID */
...
/* @bsp, 2019/09/20 Wireless Charge otg support */
		...
		/* @bsp, 2019/12/20 add pd charge statue check */
		schedule_delayed_work(&chg->pd_status_check_work,
				msecs_to_jiffies(2000));
	} else {
...
irqreturn_t default_irq_handler(int irq, void *data)
{
	...
	smblib_dbg(chg, PR_OP_DEBUG, "IRQ: %s\n", irq_data->name);
	return IRQ_HANDLED;
}
...
/* @bsp, 2019/07/05 Battery & Charging porting */
	...
	chg->dash_on = get_prop_fast_chg_started(chg);
	if (chg->dash_on) {
		chg->vbus_present = true;  /* present reset */
		pr_err("return directly because dash is online\n");
		return;
	}
...
static void norchg_check_skin_thermal_temp(struct smb_charger *chg)
{
	...
	if (call_on || skin_high || skin_warm || skin_med) {
		// 9v charging double current to battery except call.
...
bool is_fastchg_allowed(struct smb_charger *chg)
{
	...
	// both kona and kebab, no matter adapter
	...
	// only kebab (dual-cell), no matter adapter
	if (chg->swarp_supported
	...
	// both kona and kebab, only pps swarp adapter
	if (chg->swarp_online
...
static void op_handle_usb_removal(struct smb_charger *chg)
{
	/* @BSP  USB enter suspend recovery */
	union power_supply_propval pval = {0, };
	...
	if (chg->chg_ovp) {
		...
		pr_info("chg_ovp :recovery uovp for usb remove %d\n", chg->chg_ovp);
	}
...
static void set_usb_switch(struct smb_charger *chg, bool enable)
{
	...
	if (enable) {
		...
		if (chg->boot_usb_present && chg->re_trigr_dash_done) {
			...
			vote(chg->usb_icl_votable, USB_PSY_VOTER, false, 0);
			/* Limit 3A current when bootup with PD */
			if (chg->pd_active)
				vote(chg->usb_icl_votable, AICL_RERUN_VOTER,
					true, PD_PANELON_CURRENT_UA);//3A
			else
				vote(chg->usb_icl_votable, AICL_RERUN_VOTER,
...
static void retrigger_dash_work(struct work_struct *work)
{
	...
	if (chg->ck_dash_count == DASH_CHECK_COUNT) {
		...
		if (chg->swarp_online && chg->pd_9vsupported) {
			// op type-c adapter switch fastchg fail, pd charge.
			cancel_delayed_work(&chg->pd_status_check_work);
			schedule_delayed_work(&chg->pd_status_check_work,
					msecs_to_jiffies(2000));
		}
...
static int handle_batt_temp_warm(struct smb_charger *chg)
{
	...
	if ((temp_region != BATT_TEMP_WARM)
			|| chg->is_power_changed || chg->recharge_pending
#ifdef OEM_TARGET_PRODUCT_BILLIE
			|| chg->oem_lcd_is_on != chg->last_lcd_on_status
#endif
		) {
		pr_err("triggered\n");
		chg->is_power_changed = false;
...
#ifdef OEM_TARGET_PRODUCT_BILLIE
			set_chg_ibat_vbat_max(chg,
				chg->oem_lcd_is_on ? chg->ibatmax[BATT_TEMP_WARM_LCD_ON] : chg->ibatmax[BATT_TEMP_WARM],
				chg->vbatmax[BATT_TEMP_WARM]);
#else
			set_chg_ibat_vbat_max(chg,
...
#ifdef OEM_TARGET_PRODUCT_BILLIE
		/* @BSP OEM */
		chg->last_lcd_on_status = chg->oem_lcd_is_on;
#endif
		/* Update the temperature boundaries */
		chg->mBattTempBoundT0 = chg->BATT_TEMP_T0;
		chg->mBattTempBoundT1 = chg->BATT_TEMP_T1;
...
/* @bsp, CHG bringup:DISABLE_TEMP_PROTECT start */
#ifdef CONFIG_DISABLE_TEMP_PROTECT
	if ((temp <= chg->mBattTempBoundT0) || (temp > chg->mBattTempBoundT6)) {
		//rc = handle_batt_temp_warm(chg);
		/* set 4130mV and 550mA when protect */
		set_chg_ibat_vbat_max(chg, chg->ibatmax[BATT_TEMP_WARM_LCD_ON],
			chg->vbatmax[BATT_TEMP_WARM]);
		return rc;
	}
#endif
...
#ifdef OEM_TARGET_PRODUCT_BILLIE
	/* @BSP  USB enter suspend when uvp */
	union power_supply_propval pval = {0, };
#endif
...
static int get_usb_temp(struct smb_charger *chg)
{
	int ret, result, temp = 0, step_value = 0, temp_1 = 0, temp_2 = 0;
#ifndef OEM_TARGET_PRODUCT_BILLIE
	int i = 0;
#endif
...
/* @bsp, CHG bringup:DISABLE_TEMP_PROTECT start */
	...
	return;
#endif
/* @bsp, CHG bringup:DISABLE_TEMP_PROTECT end */
	...
	if (chg->dash_on) {
		...
		if (chg->disable_normal_chg_for_dash) {
			...
			if (chg->fastchg_present_wait_count >= FULL_DELAY_COUNT) {
				// flag must set false before unsuspend and enable.
				chg->disable_normal_chg_for_dash = false;
				smblib_set_usb_suspend(chg, false);
#ifdef OEM_TARGET_PRODUCT_BILLIE
				/* @BSP ODM, not enable charging when temp out of rang */
				temp_region = op_battery_temp_region_get(chg);
				if (temp_region != BATT_TEMP_HOT
						&& temp_region != BATT_TEMP_COLD)
					op_charging_en(chg, true);
...
int get_prop_chg_protect_status(struct smb_charger *chg)
{
	...
	temp_region = op_battery_temp_region_get(chg);
	//@BSP, poweroff charging display OVP and UVP warning picture
	if (chg->chg_ovp && ((vbus_val.intval >= CHG_SOFT_OVP_MV - 100) || (vbus_val.intval <= CHG_SOFT_UVP_MV + 100)))
		return PROTECT_CHG_OVP;
	/* @BSP, display battery missing picture in poweroff charging */
