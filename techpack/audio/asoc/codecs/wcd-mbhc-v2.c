...
static void wcd_mbhc_swch_irq_handler(struct wcd_mbhc *mbhc)
{
	...
	if (mbhc->mbhc_add_debounce_time_fun) {
		if (detection_type) {
		      ...
		      bool impedance_det_en)
{
	...
	int hph_swh = 0;
	int gnd_swh = 0;
	int mbhc_add_debounce_time_fun = 0;
	...
	const char *mbhc_debounce = "mbhc_add_debounce_time_fun";
	// add for use usbc mode to judge the cable status
	bool use_usbc_det = false;

	pr_debug("%s: enter\n", __func__);

	ret = of_property_read_u32(card->dev->of_node, mbhc_debounce, &mbhc_add_debounce_time_fun);
	if (ret) {
		dev_err(card->dev,
			"%s: missing %s in dt node\n", __func__, mbhc_debounce);
		mbhc_add_debounce_time_fun = 0;
	} else {
		pr_info("%s:Support control to increase the debounce time function \n", __func__);
		pr_info("%s:ebba_debounce_fun = %d\n", __func__, mbhc_add_debounce_time_fun);
	}
	...
	mbhc->st_state = false;
	mbhc->st_lpress_lock_sleep = false;
	mbhc->mbhc_add_debounce_time_fun = mbhc_add_debounce_time_fun;
