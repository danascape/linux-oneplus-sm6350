...
enum Tfa98xx_Error tfaContWriteProfile(struct tfa_device *tfa, int prof_idx, int vstep_idx) 
{
	...
	if (prof->group == previous_prof->group && prof->group != 0) {
		 ...
		 */
		if ((tfa->tfa_family == 1) ||(tfa->tfa_family != 2)||
			((tfa->tfa_family == 2) && (TFA_GET_BF(tfa, MANSTATE) >= 6)) || 
			((tfa->tfa_family == 2) && tfa98xx_get_profile_sr(tfa, prof_idx) !=  fs_previous_profile)) {
