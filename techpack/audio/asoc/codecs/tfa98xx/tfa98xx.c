...
#ifdef TFA98xx_calibrate
static ssize_t tfa98xx_cal_read(struct file *file,
char __user *user_buf, size_t count,
loff_t *ppos)
{
...
//struct tfa98xx *tfa98xx = i2c_get_clientdata(i2c);
	...
	int i;
	enum Tfa98xx_Error err;
	if(tfa98xx_global[0]==NULL||tfa98xx_global[1]==NULL)
	{
	printk("tfa98xx_global is null\n");
	return 0;
	}
	...
	ret = sprintf(str,"%d:%d,%d:%d", 34, cal_impedence[0], 35, cal_impedence[1]); 
	pr_err("integer[0]=%d integer[1]=%d string=%s\n",cal_impedence[0],cal_impedence[1],str);
	return copy_to_user(user_buf, str, ret + 1);
	}
...
#ifdef TFA98xx_F0_tracking
static ssize_t tfa98xx_F0_read(struct file *file,
char __user *user_buf, size_t count,
loff_t *ppos)
{
	...
	ret = sprintf(str,"%d:%d",gf0[0],gf0[1]); 
	pr_err("i2c addr is 0x%x\n", tfa98xx->i2c->addr);
	pr_err("integer[0]=%d integer[1]=%d string=%s\n",gf0[0],gf0[1],str);
	return copy_to_user(user_buf, str, ret + 1);
}
...
#ifdef TFA98xx_F0_tracking
static const struct file_operations tfa98xx_dbgfs_f0_fops = {
	.open = simple_open,
	.read = tfa98xx_F0_read,
	//.write = tfa98xx_state_store,
	.llseek = default_llseek,
};
#endif
...
#ifdef TFA98xx_F0_tracking
	debugfs_create_file("f0_state", S_IRUGO|S_IWUGO, tfa98xx->dbg_dir,
						i2c, &tfa98xx_dbgfs_f0_fops);
#endif
...
static void tfa98xx_container_loaded(const struct firmware *cont, void *context)
{
	...
	if (tfa98xx_container == NULL) {
		...
		printk("container file already loaded...\n");
	...
	*/
	...
	if (tfa_is_cold(tfa98xx->tfa) == 0) {
		printk("Warning: device 0x%.2x is still warm\n", tfa98xx->i2c->addr);
...
static void tfa98xx_dsp_init(struct tfa98xx *tfa98xx)
{
	...
	if (tfa98xx->dsp_fw_state != TFA98XX_DSP_FW_OK) {
		printk("Skipping tfa_dev_start (no FW: %d)\n", tfa98xx->dsp_fw_state);
		return;
	}

	if(tfa98xx->dsp_init == TFA98XX_DSP_INIT_DONE) {
		printk("Stream already started, skipping DSP power-on\n");
		return;
	}
						...
						struct snd_soc_dai *dai)
{
	...
	return 0/*snd_pcm_hw_constraint_list(substream->runtime, 0,
			     ...
			     struct snd_soc_dai *dai)
{
	...
	printk("Requested rate: %d, sample size: %d, physical size: %d\n",
	...
	printk("mixer profile:container profile = [%d:%d]\n", tfa98xx_mixer_profile, prof_idx);
...
static int tfa98xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	if (no_start) {
		printk("no_start parameter set no tfa_dev_start or tfa_dev_stop, returning\n");
		return 0;
	}
...
static ssize_t tfa98xx_sys_cal_read(struct file *filp, struct kobject *kobj,
				struct bin_attribute *bin_attr,
				char *buf, loff_t off, size_t count)
{
//struct i2c_client *i2c = file->private_data;
//struct tfa98xx *tfa98xx = i2c_get_clientdata(i2c);
	...
	ret = sprintf(str,"%d:%d,%d:%d", 2, cal_impedence[0], 2, cal_impedence[1]);
	pr_err("integer[0]=%d integer[1]=%d string=%s\n",cal_impedence[0],cal_impedence[1],str);
...
static ssize_t tfa98xx_sys_temp_read(struct file *filp, struct kobject *kobj,
				struct bin_attribute *bin_attr,
				char *buf, loff_t off, size_t count)
{
	struct device *dev = container_of(kobj, struct device, kobj);
	...
	int ret = 0;
	exTfa98xx_getf0_req(tfa98xx);
	ret = sprintf(str,"%d,%d",temp[0],temp[1]);
	memcpy(buf, str, ret + 1);
	pr_err("buf is %s\n",buf);
	return ret + 1;
...
static struct bin_attribute dev_attr_calibra = {
	.attr = {
		.name = "calibra",
		.mode = 0777,
	},
	.size = 0,
	.read = tfa98xx_sys_cal_read,
	.write = NULL,
};

static struct bin_attribute dev_attr_temp = {
	.attr = {
		.name = "temperature",
		.mode = 0777,
	},
	.size = 0,
	.read = tfa98xx_sys_temp_read,
	.write = NULL,
};
			     ...
			     const struct i2c_device_id *id)
{
	...
	if (ret)
		dev_info(&i2c->dev, "error creating sysfs files\n");
	ret = device_create_bin_file(&i2c->dev, &dev_attr_calibra);
	if (ret)
		dev_info(&i2c->dev, "error creating sysfs files\n");
	ret = device_create_bin_file(&i2c->dev, &dev_attr_temp);
	if (ret)
		dev_info(&i2c->dev, "error creating sysfs files\n");
...
static int tfa98xx_i2c_remove(struct i2c_client *i2c)
{
	...
	device_remove_bin_file(&i2c->dev, &dev_attr_calibra);
	device_remove_bin_file(&i2c->dev, &dev_attr_temp);
